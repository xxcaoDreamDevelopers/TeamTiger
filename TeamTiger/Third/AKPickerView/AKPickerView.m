...
- (void)initialize
{
    self.backgroundColor = [UIColor colorWithRed:21/255.0f green:27/255.0f blue:39/255.0f alpha:1.0f];
    ...
    self.selectBackgroundV = [[UIView alloc] initWithFrame:CGRectMake(0, 0, Screen_Width * 0.2, 100)];
    self.selectBackgroundV.backgroundColor = [UIColor whiteColor];
    
    [self addSubview:self.selectBackgroundV];
    
    //上边的渐变阴影
    UIImageView *image=[[UIImageView alloc] init];
    self.upShowImgV = image;
    [self addSubview:image];
    
    //下边的渐变阴影
    UIImageView *image1=[[UIImageView alloc] init];
    self.downShowImgV = image1;
...
- (void)layoutSubviews
{
    ...
    self.selectBackgroundV.center = self.collectionView.center;
    
    self.upShowImgV.frame = CGRectMake(0, 0, Screen_Width * 0.2, self.bounds.size.height*1/2-50);
    self.upShowImgV.image=[[UIImage imageNamed:@"downshadow.png"] stretchableImageWithLeftCapWidth:5 topCapHeight:30];
    self.downShowImgV.frame = CGRectMake(0, self.bounds.size.height-self.bounds.size.height*1/2+50, Screen_Width * 0.2, self.bounds.size.height*1/2-50);
    self.downShowImgV.image=[[UIImage imageNamed:@"upshadow.png"] stretchableImageWithLeftCapWidth:5 topCapHeight:30];
}
...
- (void)setMaskDisabled:(BOOL)maskDisabled
{
	...
	self.collectionView.layer.mask = maskDisabled ? nil : ({
		...
		maskLayer.colors = @[(id)[[UIColor redColor] CGColor],
...
//            UICollectionViewCell *selectCell = [self collectionView:self.collectionView cellForItemAtIndexPath:indexPath];
...
{
	CGSize size = CGSizeMake(Screen_Width * 0.2, 100);
//	if ([self.dataSource respondsToSelector:@selector(pickerView:titleForItem:)]) {
//		NSString *title = [self.dataSource pickerView:self titleForItem:indexPath.item];
...
- (void)setSelected:(BOOL)selected
{
    ...
    self.contentView.backgroundColor = self.selected ? self.highlightedBackGroundColor : [UIColor clearColor];
}
